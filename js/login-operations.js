...
function login(phone,password){
    ...
    $.ajax(settings).done(function (response) {
        ...
        if(errorCode==200){
            ...
            $.cookie("uid",response.response.uid, { expires : 1 });
            $.cookie("token",response.response.loginToken, { expires : 1 });
            $.cookie("contentServer",response.response.contentServer, { expires : 1 });
            //go to main content
            //console.log(response.response.uid);
            window.location.href="account.html";
        }else if(errorCode==10005){
            $(".Validform_wrong.phone").css({color:'red'})
            $(".Validform_wrong.phone").text("*用户不存在！")
            console.log("用户不存在");
        }else if(errorCode==10007){
            $(".Validform_wrong.pwd").css({color:'red'})
            $(".Validform_wrong.pwd").text("*密码不正确！")
            console.log("密码不正确");
        }
