...
getCaptcha();
// make registration
$(".pg-cont-reg-form-item-input-submt.return_password").click(function () {
        ...
        if(verpwd.toString().length>0){
            ...
            if(!isNaN(phone)&&phone.toString().length==11){
                ...
                if(verpwd===pwd){
                    ...
                    form.append("identifierToken",phone);
                    form.append("verifyCode", verficationCode);
                    form.append("newPassword", SHA256(pwd));
                    ...
                    var settings = {
                        "async": true,
                        "crossDomain": true,
                        "url": requestUrl+"/evh/user/checkVerifyCodeAndResetPassword",
                        "method": "POST",
                        "processData": false,
                        "contentType": false,
...
function getVerificationCode(){
    ...
    if(captcha.toString().length>0){
        ...
        $(".Validform_wrong.captcha").css({color:'red'});
        $(".Validform_wrong.captcha").text("请输入图形验证码");
    }
